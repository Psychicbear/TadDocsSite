...
export class Maffs {
    ...
    clamp(value, lowerBound, upperBound) {
        ...
        if (Number.isFinite(lowerBound) === false) {
            throw new Error(ErrorMsgManager.numberCheckFailed(lowerBound));
        }
        if (Number.isFinite(upperBound) === false) {
            throw new Error(ErrorMsgManager.numberCheckFailed(upperBound));
        }
     ...
     * @param {number} alpha  - Amount of interpolation.
     */
    flerp( start, end, dt, alpha )
    {
        dt /= 1000.0;
        const decay = 25.0 * (alpha);
...
Object.defineProperty(Maffs.prototype, "clamp", {
    value: Maffs.prototype.clamp,
    writable: false,
    configurable: false,
});
