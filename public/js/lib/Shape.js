...
export class Shape {
    ...
    #roundedRectangle(x, y, w, h, rounding) {
        ...
        this.#tad.context.fillStyle = this.#colour;
        this.#tad.context.strokeStyle = this.#border;
    ...
    oval(x, y, w, h = w) {
        ...
        this.#applyRotation(x, y); // Rotate about center of shape regardless of alignment
        // this.#applyRotation(oldX, oldY); // Do it the other way
        ...
        ctx.closePath();
        // if (this.#tad.debug) {
        //     // Draws crosses at weird locations, need to fix
    ...
    #roundedPolygon(coords) {
        ...
        let D, E, F;
        let [firstD, firstE, firstF] = this.#computeRoundingDEF(
            last,
            first,
            second
        );
    ...
    set state(newState) {
        ...
        if (typeof newState.shapeColour !== `string`) {
            throw new Error(
                `invalid colour state! ${
                    newState.shapeColour
                }:${typeof newState.shapeColour}`
            );
        }
        if (typeof newState.shapeBorder !== `string`) {
            throw new Error(
                `invalid border colour state! ${
                    newState.shapeBorder
                }:${typeof newState.shapeBorder}`
            );
        }
        if (typeof newState.shapeAlignmentX !== `string`) {
            throw new Error(
                `invalid x alignment state! ${
                    newState.shapeAlignmentX
                }:${typeof newState.shapeAlignmentX}`
            );
        }
        if (typeof newState.shapeAlignmentY !== `string`) {
            throw new Error(
                `invalid y alignment state! ${
                    newState.shapeAlignmentY
                }:${typeof newState.shapeAlignmentY}`
            );
        }
        if (!Number.isFinite(newState.shapeStrokeWidth)) {
            throw new Error(
                `invalid stroke width state! ${
                    newState.shapeStrokeWidth
                }:${typeof newState.shapeStrokeWidth}`
            );
        }
        if (!Number.isFinite(newState.shapeRounding)) {
            throw new Error(
                `invalid rounding state! ${
                    newState.shapeRounding
                }:${typeof newState.shapeRounding}`
            );
        }
        if (!Number.isFinite(newState.shapeStrokeDash)) {
            throw new Error(
                `invalid stroke dash state! ${
                    newState.shapeStrokeDash
                }:${typeof newState.shapeStrokeDash}`
            );
        }
        if (newState.shapeMovedByCamera === undefined) {
            throw new Error(
                `invalid moved by camera state! ${
                    newState.shapeMovedByCamera
                }:${typeof newState.shapeMovedByCamera}`
            );
        }
        if (!Number.isFinite(newState.shapeRotation)) {
            throw new Error(
                `invalid rotation state! ${
                    newState.shapeRotation
                }:${typeof newState.shapeRotation}`
            );
        }

        this.colour = newState.shapeColour;
        this.border = newState.shapeBorder;
        ...
        this.rotation = newState.shapeRotation;
    }
    ...
    set colour(value) {
        ...
        if (Paint.get(value.toLowerCase())) {
            this.#colour = Paint.get(value.toLowerCase());
    ...
    set border(value) {
        ...
        if (Paint.get(value.toLowerCase())) {
            this.#border = Paint.get(value.toLowerCase());
